    ...
    $app->post('', function (Request $request, Response $response) {
        ...
        foreach ($mission as $key => &$value) {
            ...
            if (!isset($json_data[$key]) || gettype($json_data[$key]) != gettype($value)) {
                $error_msg = "访问参数异常，可能是缺少数据参数";
                goto Bad_request;
            }
            $value = $json_data[$key];
        }
        $mission['name'] = empty($mission['name']) ? "null" : $mission['name'];
        ...
        // 异常访问出口
        Bad_request:
        return WolfBolin\Slim\HTTP\Bad_request($response, $error_msg);
    });
...
})->add(\WolfBolin\Slim\Middleware\x_user_block())
