...
function x_auth_token() {
    $result = function (Request $request, Response $response, $next) {
        ...
        // 处理Token信息
        if ($token_status) {
            $request = $request->withAttribute('X-Auth-User', $token_status['role']);
...
function x_user_block(){
    $result = function (Request $request, Response $response, $next) {
        $user = $request->getAttribute('X-Auth-User');
        if(preg_match('/root/i',$user)){
            return $next($request, $response);
        }else{
            return \WolfBolin\Slim\HTTP\Unauthorized3($response, '您没有足够的权限');
        }
    };
    return $result;
}
